...
download.file("https://www.chp.gov.hk/files/pdf/building_list_eng_20200806.pdf", "../00_original/building_list_eng_20200806.pdf")
...
for (i_file in 165:length(pdflist)){
...
# set up function to calculate day change
calculate_day_change <- function(x){
	
	# initiate with the first number
	tempplaceholder <- x[1]-0
	...
	for (i in 2:length(x)){
		...
		# save the difference
		tempplaceholder <- c(tempplaceholder, daychange)
	}
	
	# since a drop in 14-day cases is the same as having no new cases that day
	# substitute negative numbers with zero
	# cf discussions here https://stackoverflow.com/questions/11275187/replace-negative-values-by-zero
	tempplaceholder <- (abs(tempplaceholder)+tempplaceholder)/2
